...
PROFILE_GENERAL_DEFINITION = [
    {
        ...
        "questions": [
            {
                "key": "denumire_entitate",
                "label": "Denumire entitate",
                "type": "text",
                "required": True,
            },
            ...
            {
                "key": "administrator",
                "label": "Administrator / reprezentant legal",
                "type": "text",
                "required": True,
            },
            {
                "key": "beneficiar_real",
                "label": "Beneficiar real",
                "type": "text",
                "required": False,
            },
        ],
    },
    {
        ...
        "questions": [
            {
                "key": "domeniu_activitate",
                "label": "Domeniu principal de activitate",
                "type": "text",
                "required": False,
            },
            {
                "key": "nr_documente_luna",
                "label": "Numar estimat documente / luna",
                "type": "number",
                "required": False,
            },
            {
                "key": "nr_conturi_bancare",
                "label": "Numar conturi bancare",
                "type": "number",
                "required": False,
            },
            {"key": "nr_angajati", "label": "Numar angajati", "type": "number", "required": False},
            {
                ...
                "label": "Persoana responsabila transmitere documente",
                "type": "text",
                "required": False,
            },
            {
                "key": "canale_comunicare",
                "label": "Canale de comunicare",
                "type": "multi_choice",
                "required": False,
                "options": ["email", "telefon", "whatsapp", "Platforme online"],
            },
        ],
    },
]
...
KIT_DEFS = [
    {
        ...
        "documentation_url": "/docs/kit-internal_fiscal_procedures.pdf",
        "price_eur": 45.0,
    },
    {
        "code": "digital_recurring_compliance",
        "name": "Risc fiscal",
        "description": "TVA, deductibilitate, tratament fiscal. Implementare flux digital.",
        "documentation_url": "/docs/kit-digital_recurring_compliance.pdf",
        "price_eur": 45.0,
    },
    {
        "code": "tax_residency_nonresidents",
        "name": "Risc rezidenta fiscala",
        "description": "Impozit reținut la sursă, convenții fiscale, tranzacții internaționale.",
        "documentation_url": "/docs/kit-tax_residency_nonresidents.pdf",
        "price_eur": 55.0,
    },
    {
        "code": "affiliate_compliance",
        "name": "Risc Afiliati",
        "description": "Transfer pricing, tranzacții intra-grup cu părți afiliate.",
        "documentation_url": "/docs/kit-affiliate_compliance.pdf",
        "price_eur": 45.0,
    },
    {
        "code": "affiliate_identification",
        "name": "Risc extins (ESG)",
        "description": "Chestionar complet – toate cele 50 de riscuri. Environmental, Social, Governance.",
        "documentation_url": "/docs/kit-affiliate_identification.pdf",
        "price_eur": 55.0,
    },
]
...
def _build_questions_from_admin(
    version_id: int, risk_map: dict[str, Risk]
) -> list[tuple[KitQuestion, list[tuple[bool, str]]]]:
    result = []
    section_order = 0
...
def _build_questions_from_risc_extins(
    version_id: int, risk_map: dict[str, Risk]
) -> list[tuple[KitQuestion, list[tuple[bool, str]]]]:
    result = []
...
def _seed_products_catalog(db: Session) -> None:
    ...
    existing = {
        (row.bundle_product_id, row.kit_id)
        for row in db.query(BundleInclude)
        .filter(BundleInclude.bundle_product_id == bundle.id)
        .all()
    }
    for kit in kits:
...
def _seed_kit_from_admin(
    db: Session, version: KitVersion, kit: Kit, risk_map: dict[str, Risk]
) -> None:
        ...
        for q_idx, q in enumerate(sec["questions"]):
            ...
            for code in q.get("trigger_nu", []):
                ...
                if r:
                    db.add(
                        QuestionRiskMap(
                            question_id=question.id, risk_id=r.id, trigger_on_true=False
                        )
                    )
            for code in q.get("trigger_da", []):
                r = risk_map.get(code)
                if r:
                    db.add(
                        QuestionRiskMap(question_id=question.id, risk_id=r.id, trigger_on_true=True)
                    )


def _seed_kit_from_risc_extins(
    db: Session, version: KitVersion, kit: Kit, risk_map: dict[str, Risk]
) -> None:
...
def _seed_kit_generic(
    db: Session, version: KitVersion, kit: Kit, risk_map: dict[str, Risk], code: str
) -> None:
